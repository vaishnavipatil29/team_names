...
<body bgcolor = "#006973">
	<image src = "Finder_Homepage_Background_1.jpg", id = "back_img1", class = "back_img1"></image>
	<input type = "button" class = "main1" value = "Recommended for you" onclick="go_below()"></input>
	<a href = "search.html"><input type = "button" class = "main2" value = "Search by criteria"></input></a>
	<div class = "arrow_down" onmouseover = "go_below()"></div>
	<?
		$mysqli = new mysqli("localhost", "root", "", "A_New_Tinder");
	    $mysqli->query("USE A_New_Tinder");
		$result = $mysqli->query("SELECT * FROM Recommended_" . $_SESSION['ID'] . "");
			if($result->num_rows > 0)
            	{
            		$x=1;
                	while($r = $result->fetch_assoc())
                	{
                		$res = $mysqli->query("SELECT * FROM Artists WHERE ID=\"" . $r['ID'] . "\"");
							if($res->num_rows > 0)
							{
								$row = $res->fetch_assoc();
	...
	<table class = <? echo "\"box".$x."\""; ?>>
		<tr>
			<td>
				<center>
					<image src = <? echo "\"images/" . $row['Image'] . "\""; ?> class = "finderphoto"></image>
				</center>
			</td>
		</tr>
		<tr>
	...
	<div class = "dropdown">
		...
		<div class = "dropdown-content">
			<a href = "finder_profile.php">Your Profile</a>
