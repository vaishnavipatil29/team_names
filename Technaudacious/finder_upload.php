	...
	<?
		...
		else
		{
		    if (move_uploaded_file($_FILES["filetoupload"]["tmp_name"], $target_file))
		    {
		    	...
		    	$result = $mysqli->query("UPDATE Finders SET Image = \"" . $file_name . "\" WHERE ID=\"" . $_SESSION['ID'] . "\"");

				echo "<br><form>";
				echo "<input type=\"button\" value=\"Back\" name=\"back\" id=\"back\" onclick=\"call_album()\">";
				echo "</form>";
				$res = $mysqli->query("SELECT ID, Email, Password, State FROM Finders WHERE ID=\"" . $_SESSION['ID'] . "\"");
        		if($res->num_rows > 0)
            	{
	                $row = $res->fetch_assoc();
